...
public class CircleType1_Up : MonoBehaviour
{
    [Header("구가 일정 위치까지 올라가고 난 후 터짐")]
    ...
    private void FixedUpdate()
    {
        ...
        transform.Translate(Vector2.up * Speed * Time.deltaTime * Slow);
        Up();
    }
    private void Up()
    {
        if (transform.position.y >= minHieght)        //y 최소값보다 커지면 오브젝트 슬로우
        {
            onSlow = true;
        }

        if (transform.position.y >= maxHieght)       //y 최대값 보다 커지면 오브젝트 산개
        {
            onEnd = true;
        }
